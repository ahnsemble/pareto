...
fn apply_hero_conditionals(input: &Value, stats: &mut Map<String, Value>, applied: &mut Vec<String>) {
    let hero_id = input
        .get("selected_hero")
    ...
    if hero_id == "venato" && bool_value(conditional, "skill_active_window") {
        add_stat(stats, "adrenaline", num(conditional, "hp_missing_ratio") * 40.0);
...
fn apply_ss_effects(input: &Value, stats: &mut Map<String, Value>) -> Vec<String> {
    ...
    if equipment_by_id(input, "moonscarBracer").is_some() {
        ...
        if stat(stats, "critRate") >= 1.0 {
            add_stat(stats, "critDamage", if dragon_gold >= 8.0 { 60.0 } else { 30.0 });
        }
        if stat(stats, "critRate") >= 1.3 {
            add_stat(stats, "skillDamage", if dragon_total >= 25.0 { 60.0 } else { 30.0 });
        }
    ...
    if let Some(boots) = equipment_by_id(input, "glacialWarboots") {
        add_stat(
            stats,
            "shieldDamage",
            if num(boots, "astral_forge_eaf_level") >= 5.0 { 60.0 } else { 50.0 },
        );
        if num(boots, "chaos_fusion_level") >= 6.0 {
            add_stat(stats, "glacialBloodline", if num(boots, "chaos_fusion_level") >= 10.0 { 30.0 } else { 15.0 });
        }
    }
    ...
    if equipment_by_id(input, "voidwakerEmblem").is_some() {
        set_stat_max(stats, "voidNeckBoost", if str_value(input, "mode") == Some("ee") { 1.4 } else { 1.2 });
        set_stat_max(stats, "voidNeckBoostUptime", 1.0);
    }
    effects.push(SS_EFFECT_IDS[6].to_string());

    if equipment_by_id(input, "voidwakerHandguards").is_some() || equipment_by_id(input, "moonscarBracer").is_some() {
        set_stat_max(stats, "voidGlovesInstakill", 1.05);
    }
    effects.push(SS_EFFECT_IDS[7].to_string());

    if equipment_by_id(input, "voidwakerTreads").is_some() || equipment_by_id(input, "glacialWarboots").is_some() {
    ...
    set_stat_max(stats, "chaosBeltBoost", 1.0 + stat(stats, "chaosBeltBoost") / 100.0);
    set_stat_max(stats, "hpBulletBoost", 1.0 + stat(stats, "hpBulletBoost") / 100.0);
...
fn multiplier_stages(input: &Value, stats: &Map<String, Value>) -> Vec<f64> {
    ...
    vec![
        atk_base
            ...
            + atk_final,
        stat(stats, "critRate") * stat_or(stats, "critDamage", 1.0) + (1.0 - stat(stats, "critRate")),
        1.0 + stat(stats, "vulnerability").max(0.0) / 100.0,
        1.0 + (stat(stats, "shieldDamage") * stat(stats, "shieldDamageUptime")).max(0.0) / 100.0,
        1.0
            + ((stat(stats, "poisoned") * stat(stats, "poisonedUptime")).max(0.0)
        ...
        1.0
            + ((stat(stats, "laceration") * stat(stats, "lacerationUptime")).max(0.0)
